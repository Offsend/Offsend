...
pub fn merge_exclude_lists(existing: &[String], additional: &[String]) -> (Vec<String>, Vec<String>) {
...
mod tests {
    ...
    fn resolve_csv_and_dedupe() {
        let ids = resolve(&["node,swift".to_string(), "swift".to_string(), "tuist".to_string()])
            .unwrap();
        assert_eq!(
            ids,
            vec![
