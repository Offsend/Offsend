...
public final class DocumentProcessingPipeline: DocumentProcessing, Sendable {
    ...
    /// App / production must inject Rust wrappers (`RustPreferringDetector`,
    /// `RustRiskScoringEngine`, `RustMaskingEngine`).
    public init(
        textExtractor: DocumentTextExtractor = DocumentTextExtractor(),
        detector: SensitiveDataDetecting,
        riskScorer: RiskScoring,
        maskingEngine: TextMasking,
