...
final class FileLocalStoreTests: XCTestCase {
    ...
    func testLocalStoreFactoryUsesFileStoreOnLinux() throws {
        let masking = PassthroughTextMasking()
        #if os(Linux)
        let store = try LocalStoreFactory.makeDefaultStore(maskingEngine: masking)
...
/// Minimal `TextMasking` for StorageCore tests (production App injects `RustMaskingEngine`).
private struct PassthroughTextMasking: TextMasking {
    func mask(text: String, entities: [SensitiveEntity], ttl: MappingTTL) -> MaskingResult {
        MaskingResult(maskedText: text, mapping: [:], retention: .ephemeral)
    }
