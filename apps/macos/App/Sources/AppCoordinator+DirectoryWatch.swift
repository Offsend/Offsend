...
extension AppCoordinator {
    ...
    func runWatchAudit(
        ...
        watchAuditTasks[watchID] = Task { @MainActor in
            ...
            let result = await Task.detached(priority: .utility) { () -> AIWorkspacePrivacyAuditResult in
                // Incremental path: always full Rust audit. UI delta is derived later via
                // `AIWorkspacePrivacyAuditDelta.compute` from the previous cached result.
                do {
                    return try RustPrivacyBridge.auditAndMap(
                        directory: rootURL,
    ...
    func applyWorkspaceWatchFixFromNotification(watchID: UUID) {
        ...
        let configuration = directoryCheckAuditConfiguration()
        let auditResult: AIWorkspacePrivacyAuditResult
        if let cached = directoryWatchRuntime.lastResultByWatchID[watchID] {
            auditResult = cached
        } else {
            do {
                auditResult = try RustPrivacyBridge.auditAndMap(
                    directory: rootURL,
                    configuration: configuration
                )
            } catch {
        ...
        let fixResult: AIWorkspacePrivacyFixResult
        do {
            fixResult = try RustPrivacyBridge.fixAndMap(
                directory: rootURL,
            ...
            fixResult = AIWorkspacePrivacyFixResult(
                createdRelativePaths: [],
                updatedRelativePaths: [],
                errors: [
                    AIWorkspacePrivacyAuditError(
                        id: "rust-privacy-fix-failed",
                        message: error.localizedDescription
                    )
                ]
            )
