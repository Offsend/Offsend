...
public enum ProjectConfigTemplates {
    ...
    /// Detectors disabled by `offsend init` — PII/noise only. Secret/credential
    /// detectors stay enabled (see `credentialDetectorIDs`).
    public static let defaultDisabledDetectorIDs: [String] = [
        "email",
        "phone",
        "money",
        "url",
    ...
    public static let credentialDetectorIDs: [String] = [
        "apiKeyGeneric",
        "openAIAPIKey",
        "awsAccessKeyId",
        "githubToken",
    ...
    ///
    /// When `strictCredentials` is true, enables policy checks in check/hooks and
    /// adds a tighter `context` block. Does not change the default local soft-block
    /// editor hook policy — reinstall with `--hook-policy block` when desired.
    public static func renderYAML(
        templates: [ProjectConfigTemplateID],
        ignoreCommit: Bool = false,
        hooksPublish: Bool = false,
        strictCredentials: Bool = false
    ) -> String {
        ...
        let generatedBy: String = {
            ...
            if extras.isEmpty {
                parts.append("offsend init")
            } else {
                parts.append("offsend init --template \(extras.joined(separator: ","))")
            }
            if strictCredentials {
                parts.append("--strict-credentials")
            }
        ...
        let policyValue = strictCredentials ? "true" : "false"
        let disableLines = defaultDisabledDetectorIDs.map { "              - \($0)" }.joined(separator: "\n")
        let contextSection = strictCredentials
            ? """
