...
The shell-gate does not read stdout, and it does not try to predict what a command will read. A command that enumerates files without naming a sensitive path — `find … -exec cat`, a recursive `grep`, an interpreter one-liner that walks the tree — is allowed, and that is a stated non-goal rather than a missing rule. Secrets a command prints are caught after the fact by the shell-output audit (Cursor `afterShellExecution`, Claude `PostToolUse Bash`), which logs the command and the detector type names to a `0600` user-local file and notifies the user. Because neither editor accepts a replacement for terminal output, that audit is a rotation signal, not prevention: the value is already in model context when it fires. Content gates additionally decode-probe contiguous and commonly wrapped base64/hex runs when the agent later reads terminal artifacts or other buffers, so encoded secret dumps read back from disk can still deny/seal. Probe-budget overflow denies/withholds instead of allowing an unscanned tail. Runtime encoding (`chr()`, `eval`, unresolved variables) and renamed copies without detectable content remain residual; keep secrets off disk as the primary control, and use an OS sandbox when a read or an egress must actually be prevented.

### Shell-gate rule admission
...
What the shell-gate declines to do, an OS sandbox can: `sandbox.enabled: true` in `.offsend.yml` is materialized by `offsend sync` into each detected editor's own sandbox configuration — nono for CLI agents where it is installed, otherwise the editor's native mechanism. This satisfies the admission rule by construction rather than by careful tables: the sandbox is default-deny, is enforced by the kernel, and once applied can only be tightened.

Only egress denial is portable. Denying reads of named paths exists in Claude Code and nono but not in Cursor or Codex, where "read-only" means read everywhere, so `enabled: true` does not promise the same filesystem protection everywhere; `offsend doctor` prints the position each mechanism actually reached. Egress is nevertheless the risk the deleted signatures were written for — exfiltration requires leaving the machine.

`enabled: true` tightens and applies without a trusted policy, while `enabled: false`, `network.default: allow`, and `network.allow` entries loosen and are ignored until `offsend policy trust`: an agent editing `.offsend.yml` can neither disable its own sandbox nor authorize its own egress route. Two behaviors are fixed rather than configurable: Claude's `allowUnsandboxedCommands` is forced to `false`, because a command retried outside the sandbox makes the guarantee theatre, and exactly one mechanism owns filesystem isolation, because nested Seatbelt profiles conflict — so choosing nono switches Claude's own sandbox off deliberately. `offsend check --policy` and `doctor` fail on drift and on the four values that keep a sandbox nominally enabled while removing its point: `insecure_none`, `allowUnsandboxedCommands: true`, `filesystem.disabled: true`, `sandbox_mode = "danger-full-access"`.

Offsend generates and verifies; it does not enforce. Hooks run inside an already-started agent while a sandbox applies at process launch, so for nono Offsend writes the profile and `offsend run` (or the printed `nono run`) launches through it. What it can do is refuse to work unsandboxed: with a sandbox policy in force, a command the editor reports as unsandboxed follows `context.shell.mode`. Cursor as an IDE cannot be wrapped at all, `~/.codex/config.toml` lives outside the repository and is reported rather than written, and Windsurf has no sandbox.
...
Known residual gaps include dynamic Git/daemon/environment operations hidden behind `eval`, process APIs, variables, generated scripts, MCP tools, custom binaries, remote contexts, undiscovered sockets, or already-poisoned parent environments; editor paths not mediated by a supported pre-tool hook (Codex has no file/MCP gates; Windsurf cannot seal MCP responses); ungated Claude subagents; Cursor Grep without seal mode; some Cursor open-tab paths; cloud agent sessions; and secrets already written to local transcripts — use `offsend history audit` / `scrub` for the last case. Use patched editor versions (Cursor 3.0 or newer for the workspace-hook issue described in CVE-2026-48124). Details: [docs/cli.md → What hooks cover](docs/cli.md#what-hooks-cover). `offsend doctor` checks CLI, key, hooks, ignore drift, and sandbox — it does not enumerate every residual above.
