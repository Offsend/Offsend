...
| `--shell-gate` | — | Sensitive-path gate for Cursor / Claude shell hooks (`ask` on findings) |
...
| `--shell-gate` / `--no-shell-gate` | Shell-command gate (**Cursor + Claude only**). **On by default**; lower-risk findings ask, control-plane findings deny. Cursor `beforeShellExecution` with `failClosed: true`. `--no-shell-gate` disables |
...
| Agent shell (`Bash` / `beforeShellExecution`) | Shell-gate | On by default for Cursor/Claude; sensitive paths, safe PATH/HOME overrides, and lower-risk daemon mutations return `ask`; control-plane, environment injection, container execution, and direct privileged-socket operations hard-deny |
