...
No. `.gitignore` controls Git; Offsend controls AI context. Rules live in `.offsend.yml`, and `offsend sync` maintains the AI ignore files (`.cursorignore`, `.claudeignore`, …) from it. For Claude Code, `sync` also merges those patterns into `permissions.deny` in `.claude/settings.json` (Read/Edit/Write) — `.claudeignore` is not a Claude Code feature. When `ignore.commit` is `false` (default), sync also adds those AI ignore paths to `.gitignore` so they stay untracked. Commit `.offsend.yml` so the team shares one boundary — see [team setup](team.md).

**Do I edit `.cursorignore` / `.claudeignore` by hand?**  
You can — lines outside the offsend managed block are preserved. Prefer `offsend ignore <pattern>` or editing `ignore.patterns` in `.offsend.yml` (seeded with AI privacy defaults at `init`), then `offsend sync`, so the rule is shared across every tool. If ignore files drift from `.offsend.yml`, `offsend doctor`, `offsend show`, and `offsend check --policy` report it; CI with `fail-on: block` fails on that drift. Claude's path lock is `permissions.deny` in `.claude/settings.json` (plus hooks), not `.claudeignore`. `sync` still writes `.claudeignore` for other tools that read it.

**Does Claude Code honor `.claudeignore`?**  
No. Anthropic does not treat `.claudeignore` as a Claude Code ignore file. The project boundary for Claude is `permissions.deny` in `.claude/settings.json` (applied to the session and to Agent subagents) together with Offsend hooks. `offsend sync` writes both: deny rules from `ignore.patterns`, and `.claudeignore` for other tools. CI `check --policy` does not fail when `.claude/settings.json` is missing on the runner (`hooks.publish` defaults to `false`).
...
Partly. It also checks AI-context boundaries: what AI tools can read, whether ignore rules exist or have drifted, MCP exposure, and local agent history. It is not a GitGuardian detector-count competitor — see [Offsend and GitGuardian](../README.md#offsend-and-gitguardian).

**How is Offsend different from GitGuardian ggshield?**  
ggshield’s AI hooks **block** and send the scan to GitGuardian’s cloud; without their API key they **fail open**. Offsend scans **locally**, **seals** secrets so the agent keeps working (`{{TYPE:v1.…}}` tokens; restore with `offsend unseal`), and **withholds** secret-bearing MCP output if there is no seal key. Team rules live in `.offsend.yml` in git. Offsend has no Copilot runtime and no Windows CLI. Details: [README → Offsend and GitGuardian](../README.md#offsend-and-gitguardian).

**Can Offsend and ggshield hooks run together?**  
Yes. `offsend setup` and `offsend hook install` **merge** into existing editor configs (`~/.cursor/hooks.json`, `.claude/settings.json`, …) and do not remove foreign hooks. Offsend is not a ggshield uninstaller. Git hooks are different: a foreign (non-Offsend) `.git/hooks/pre-commit` is skipped unless you pass `--force`.
...
No for path exposure — paths and ignore rules only. Optional transcript *counts* (or a content scan with `context.history.scan_in_show` / `offsend show --scan-history`). Content scanning of paths is `offsend check`; of agent history is `offsend history audit`.

**Which platforms?**  
App: macOS 13+. CLI: macOS and Linux (x86_64 / arm64). Action: Linux and macOS runners.

**Which AI tools?**  
Ignore files from one `.offsend.yml` cover Cursor, Claude, Codex, Windsurf, plus Copilot, Continue, Gemini, and similar. Runtime depth differs:

| Editor | Runtime gates |
| --- | --- |
| Cursor, Claude Code | Prompt, read, write, shell, MCP args; MCP response seal |
| Windsurf | Prompt, read, write, shell, MCP args; no MCP response seal |
| Codex | Prompt only — no file / MCP gates yet |

`offsend setup` installs user-level Cursor/Claude hooks (any folder). Windsurf/Codex gates come from `offsend sync` in a repo. Extension chats: ChatGPT, Claude, Gemini, Grok, Perplexity, DeepSeek. Multi-tool support exists so one committed policy follows the repo — not because switching editors is the goal.

**Can Offsend check prompts before they reach an AI editor?**  
Yes. After clone or init, `offsend sync` installs git hooks from [`hooks.git`](configuration.md#hooksgit) (default `[pre-commit]`) plus detected AI-editor hooks (when [`hooks.enabled`](configuration.md#hooksenabled) is true, the default). Machines also get user-level Cursor/Claude hooks from `offsend setup`. For a specific editor in the repo: `offsend hook install --target cursor` (or `claude`, `windsurf`, `codex`, `all`). `doctor` **fails** if declared git hooks are missing while `hooks.enabled` is true; project AI-editor files are a warning. CI `check --policy` does not require git hooks or `.cursor/hooks.json` on the runner. `offsend init` includes `post-merge` in `hooks.git` so a pull that updates `.offsend.yml` re-runs `sync` automatically.

**Can the agent keep working when a read is denied because of secrets?**  
Yes, with seal-for-agents: set `context.read.on_secret: seal` (plus a seal key via `offsend setup` or `offsend keygen --default`). The read-gate still denies the original file but hands the agent a sealed copy where findings are `{{TYPE:v1.…}}` tokens. The user restores agent outputs with `offsend unseal` (copy the tokens, then run it — a terminal with no args reads the clipboard); the shell-gate blocks (default `context.shell.mode: deny`) or asks (`mode: ask`) before the agent runs `unseal` itself. Seal / MCP-seal scans ignore `check.detectors.disable` so enabled detector classes and custom dictionaries cannot silently remain plaintext; fuzzy `highEntropyString` remains excluded. Large, wrapped, and multiple base64/hex blobs in files the agent reads (including terminal transcripts) are decode-probed; scan-budget overflow denies/withholds instead of allowing an unscanned tail.

**Are AI-editor hooks a hard block on every way to read a file?**
No. They are defense-in-depth on known editor paths (prompt, `@file`, Read/Edit/Write, shell, MCP tool **args** + **responses**, Cursor subagent tasks, Cursor Grep under seal). Prefer `offsend protect` / AI ignore files first (Claude: `permissions.deny` in `.claude/settings.json`). For enumeration/egress that hooks cannot decide from command text, use [`sandbox.enabled`](configuration.md#sandbox). Other gaps remain: Claude subagent **hooks** (path deny still applies), cloud sessions, renamed copies without secret-shaped content, and secrets already in local transcripts (`offsend history audit` / `scrub`). See [what hooks cover / do not cover](cli.md#what-hooks-cover).
