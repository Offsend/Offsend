...
enum ScanJobRunner {
    static func run(parameters: ScanRepositoryJobParameters, services: ScanServices) async {
        ...
        do {
            ...
            let toolVersion = services.toolVersion
            let report = try await withDeadline(services.scanTimeout) {
                try scanner.scan(directoryURL: cloneDestination, toolVersion: toolVersion)
            }
    ...
    private static func withDeadline<T: Sendable>(
        ...
        let work = Task.detached {
            Result(catching: { try operation() })
        }
        let result: Result<T, Error>? = await withTaskGroup(of: Result<T, Error>?.self) { group in
