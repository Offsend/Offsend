...
enum TestSupport {
    ...
    static func sampleReportJSON(
        ...
        exposedFiles: Int = 2,
        errors: [String] = [],
        fixFiles: [[String: String]] = [],
        includeFixFilesKey: Bool = true,
        rules: [[String: Any]] = [
            [
                "id": "cursor-ignore",
                "title": ".cursorignore",
        ...
        let rulesJSON = rules.map { rule -> String in
            let id = rule["id"] as! String
            let title = rule["title"] as! String
            let toolName = rule["toolName"] as! String
            let severity = rule["severity"] as! String
            let scans = rule["scansForSensitivePatterns"] as! Bool
            let remediation = rule["remediation"] as! String
            return """
            {
              "id": "\(id)",
              "title": "\(title)",
              "toolName": "\(toolName)",
              "severity": "\(severity)",
              "scansForSensitivePatterns": \(scans),
              "remediation": "\(remediation)"
            }
            """
        }.joined(separator: ",\n          ")

        let patternMetaJSON = patterns.map { pattern -> String in
            """
            {
              "id": "\(pattern["id"]!)",
              "title": "\(pattern["title"]!)",
        ...
        if includeFixFilesKey {
            let items = fixFiles.map { file -> String in
