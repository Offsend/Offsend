...
**AI prompt gates** — `offsend hook install --target cursor|claude|windsurf|codex` checks prompts via editor hooks before they leave the IDE (read-gate and shell-gate on by default for Cursor/Claude). Defense-in-depth — see [what hooks cover / do not cover](docs/cli.md#what-hooks-cover).
...
offsend hook install --target cursor --no-shell-gate  # opt out of the shell gate
